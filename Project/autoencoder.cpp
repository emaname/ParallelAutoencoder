...
void Autoencoder::train(double* data) 
{
	...
	encoderWeights = (double *) malloc(m_dataDimension * m_hiddenDimension * sizeof(double));
	decoderWeights = (double *) malloc(m_dataDimension * m_hiddenDimension * sizeof(double));

	int k = 0;

	for(int i = 0; i < m_hiddenDimension; i++) {
		for(int j = 0; j < m_dataDimension; j++){
			encoderWeights[k] = m_encoderWeights[i][j];
			k++;
		}
	}
	...
	for(int i = 0; i < m_dataDimension; i++) {
		for(int j = 0; j < m_hiddenDimension; j++){
			decoderWeights[k] = m_decoderWeights[i][j];
			k++;
		}
	}
}
...
void Autoencoder::report(int id, int epoch) const
{
	cout.precision(10);
	cout << fixed;
	cout << "Process " << id << ") Epoch: " << epoch << " Error: " << m_error << endl;
}